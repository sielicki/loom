...
inline constexpr const std::string_view raw_type_name = get_template_function_name_use_decay_type<
...
struct fixed_string {
    ...
    [[nodiscard]] friend constexpr bool operator==(const fixed_string&,
                                                   const fixed_string<CharT, N2>&) {
        return false;
    }
};
...
#    endif
    ...
    static inline std::string query_pattern;
    static inline bool invert_query_pattern = false;
    static inline std::string query_regex_pattern;
    static inline bool show_help = false;
    static inline bool show_tests = false;
    static inline bool list_tags = false;
    static inline bool show_successful_tests = false;
    static inline std::string output_filename;
    static inline std::string use_reporter = "console";
    static inline std::string suite_name;
    static inline bool abort_early = false;
    static inline std::size_t abort_after_n_failures = std::numeric_limits<std::size_t>::max();
    static inline bool show_duration = false;
    static inline std::size_t show_min_duration = 0;
    static inline std::string input_filename;
    static inline bool show_test_names = false;
    static inline bool show_reporters = false;
    static inline std::string sort_order = "decl";
    static inline std::size_t rnd_seed = 0;
    static inline std::string use_colour = "yes";
    static inline bool show_lib_identity = false;
    ...
    static inline const std::vector<option> options = {
        ...
        {"--libidentify",
         "",
         std::ref(show_lib_identity),
         "report name and version according to libidentify standard"},
        {"--wait-for-keypress",
         "<never|start|exit|both>",
         std::ref(wait_for_keypress),
        ...
        for (auto i = 1U; i < n_args && argv != nullptr; i++) {
            ...
            if (std::holds_alternative<std::reference_wrapper<std::size_t>>(var)) {
                ...
                if (last != argument.length()) {
                    std::cerr << "cannot parse option of " << argv[i - 1] << " " << argv[i]
                              << std::endl;
                    std::exit(-1);
                }
...
class printer {
    [[nodiscard]] auto color(const bool cond) { return cond ? colors_.pass : colors_.fail; }

public:
    printer() = default;
    printer(const colors colors) : colors_{colors} {}
    ...
    auto& operator<<(const detail::and_<TLhs, TRhs>& op) {
        return (*this << color(op) << '(' << op.lhs() << color(op) << " and " << op.rhs()
                      << color(op) << ')')
               << colors_.none;
    }
    ...
    auto& operator<<(const detail::or_<TLhs, TRhs>& op) {
        return (*this << color(op) << '(' << op.lhs() << color(op) << " or " << op.rhs()
                      << color(op) << ')')
               << colors_.none;
    }
    ...
    auto on(events::summary) -> void {
        if (tests_.fail or asserts_.fail) {
            printer_ << "\n========================================================"
                     ...
                     << printer_.colors().fail << tests_.fail << " failed" << printer_.colors().none
                     << '\n'
...
class reporter_junit {
    ...
    void check_for_scope(std::string_view test_name) {
        ...
        const auto [iter, inserted] = active_scope_->nested_tests->try_emplace(
            str_name,
            test_result{active_scope_, detail::cfg::executable_name, active_suite_, str_name});
        ...
        if (!inserted) {
            std::cout << "WARNING test '" << str_name << "' for test suite '" << active_suite_
                      << "' already present\n";
        }
    }
    ...
    void pop_scope(std::string_view test_name_sv) {
        ...
        ss << "not popping because '" << active_test_.top() << "' differs from '" << test_name
           << "'" << std::endl;
#    if defined(__cpp_exceptions)
...
protected:
    void print_duration(auto& printer) const noexcept {
        if (detail::cfg::show_duration) {
            std::int64_t time_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
    ...
    void print_console_summary(std::ostream& out_stream, std::ostream& err_stream) {
        for (const auto& [suite_name, suite_result] : results_) {
            if (suite_result.fails) {
                ...
                out_stream << color_.pass << "\nSuite '" << suite_name << "': all tests passed"
...
class runner {
    ...
    public:
        constexpr filter(std::string_view _filter = {}) : path_{utility::split(_filter, delim)} {}
        ...
        if (not value_) {
            ...
            if constexpr (requires {
                              requires std::invocable<TMsg> and
                                           not std::is_void_v<std::invoke_result_t<TMsg>>;
                          }) {
    ...
    return [f, t](std::string_view type, std::string_view name) {
        for (int counter = 1; const auto& arg : t) {
            detail::on<F>(events::test<F, decltype(arg)>{
                .type = type,
                .name = std::string{name} + " (" + format_test_parameter(arg, counter) + ")",
                .tag = {},
                .location = {},
                .arg = arg,
                .run = f});
...
[[nodiscard]] constexpr auto operator|(const F& f, const T<Ts...>& t) {
    constexpr auto unique_name =
        []<class TArg>(std::string_view name, const TArg& arg, int& counter) {
...
public:
    ...
    constexpr steps(const TSteps& _steps) : steps_{_steps} {}
...
__attribute__((constructor(101))) inline void cmd_line_args(int argc, const char* argv[]) {
