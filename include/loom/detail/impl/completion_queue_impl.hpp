...
inline auto completion_queue::poll() const -> std::optional<completion_event> {
    ...
    if (ret < 0) {
        ...
        ssize_t err_ret = ::fi_cq_readerr(impl_->cq, &err_entry, 0);

        completion_event event{};
        if (err_ret < 0) {
            // Could not read error entry - return generic error based on original return
            event.error = make_error_code_from_fi_errno(static_cast<int>(-ret));
            return event;
        }
...
inline auto completion_queue::get_wait_fd() const -> result<int> {
    ...
    int fd = -1;
    int ret = ::fi_control(&impl_->cq->fid, FI_GETWAIT, &fd);
    if (ret != 0) {
        return make_error_result_from_fi_errno<int>(-ret);
    }

    if (fd < 0) {
        return make_error_result<int>(errc::not_supported);
    }
