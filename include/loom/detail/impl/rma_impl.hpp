...
inline auto inject_write(endpoint& ep, std::span<const std::byte> data, remote_memory remote)
    -> result<void> {
...
inline auto get_max_rma_size(const endpoint&) -> std::size_t {
    return 1ULL << 30;
}

inline auto get_inject_size(const endpoint&) -> std::size_t {
    return 4096;
}
